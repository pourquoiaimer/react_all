...
const Weather = () => {
    ...
    useEffect(() => {
        fetch('https://opendata.cwa.gov.tw/api/v1/rest/datastore/F-C0032-001?Authorization=CWA-23ED0872-47D9-4D97-A9BF-A6182904FD12&format=JSON&locationName=')
            .then((response) => {
            ...
            .then((response) => {
                ...
                let now_time = response.records.location[0].weatherElement[0].time[0].startTime.split(" ")[1].split(":")[0] == 18 ? "今天晚上" : "今天白天"
        ...
        const data = [
            {
                ...
                "最低溫度": nowWeatherData.weatherElement[2].time[0].parameter.parameterName,
                "最高溫度": nowWeatherData.weatherElement[4].time[0].parameter.parameterName,
            },
            {
                "name": nowTime == "今天白天" ? "今天晚上" : "明天白天",
                "天氣": nowWeatherData.weatherElement[0].time[2].parameter.parameterName,
                "最低溫度": nowWeatherData.weatherElement[2].time[1].parameter.parameterName,
                "最高溫度": nowWeatherData.weatherElement[4].time[1].parameter.parameterName,
            },
            {
                "name": nowTime == "今天白天" ? "明天白天" : "明天晚上",
                "天氣": nowWeatherData.weatherElement[0].time[2].parameter.parameterName,
                "最低溫度": nowWeatherData.weatherElement[2].time[2].parameter.parameterName,
                "最高溫度": nowWeatherData.weatherElement[4].time[2].parameter.parameterName,
            }
        ]
            ...
            {weatherData != "" ?
                <>
                    {city_list.length != 0 &&
                        <div >
                            {/* <div>{city_list}</div> */}
                            <select name="" id="city_list" onChange={(e) => { change_city(e) }}>
                                <option value="none" selected disabled hidden>請選擇城市</option>
